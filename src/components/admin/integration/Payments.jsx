...
export default function Payments() {
  ...
  const toggle = async () => {
    try {
      ...
      alert('Failed to update Payments integration.')
    }
  }

  return (
    <div className="space-y-4">
      <h1 className="text-lg font-semibold text-slate-900">Payments Integration</h1>
      <Card className="bg-white border border-slate-200 rounded-2xl shadow-sm max-w-xl">
        <CardContent className="p-4 space-y-3">
          <p className="text-sm text-slate-600">
            Control whether third-party payment gateways are active.
          </p>
